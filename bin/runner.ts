...
export async function run(config: MergedConfigurationModel) {
  ...
  if (config.files && config.files.length) {
    ...
    const reversedPrecedenceScopes = precedenceScopes.reverse()
...
function checkExtAndSaveFileScopeMapIfNeed(filesGlobMatchedMap: Map<string, ScopeNameAndExt>, supportedExtensions: string[], file: string, name: string) {
 ...
 */
function processFiles(fileScopeMap: Map<string, ScopeNameAndExt>, precedenceScopeMap: Map<string, ScopeWithId>, getRender: (ext: string) => RenderFunc) {
  return Promise.map(Array.from(fileScopeMap.entries()), ([file, conf]) => {
    ...
    return readFile(file, 'utf8').then(content => {
      ...
      if (renderResult.status === RENDER_STATUS.update && scope.updateStrategy === 'skip' || renderResult.status === RENDER_STATUS.identical) {
...
function loadTemplates(scopes: ScopeWithId[]) {
  return Promise.map(scopes, (scope => {
    if (scope.licenseTemplateUrl) {
      return readFile(scope.licenseTemplateUrl, 'utf8')
        .then(content => ({ ...scope, licenseText: content }))
    } else {
